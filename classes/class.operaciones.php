...
class Operaciones {
    ...
    public function usuario_login($usuario, $password, $idusuario = 0) {
        ...
        try {
            ...
            //$sql = sprintf("select idusuario, idcategoria, usuario, nombres, apellidos, cargo, email, idcliente, last_login from perfiles where usuario = '%s' and password = md5('%s')",
    ...
    public function perfil_completo($idusuario) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
    ...
    public function usuario_datos($idusuario) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            mysql_select_db($base, $conex);
            //$sql = sprintf("select t1.idusuario, t1.idcategoria, t2.nombre as categoria, t1.usuario, t1.nombres, t1.apellidos, concat(t1.nombres,' ',t1.apellidos) as fullname, t1.cargo, t1.email, t1.idcliente, t3.nombre as nombrecliente, t1.idagente, concat(t4.nombres, ' ', t4.apellidos) as agente, t1.last_login from perfiles t1 left join usuarios_categoria t2 on t1.idcategoria = t2.idcategoria left join clientes t3 on t3.idcliente = t1.idcliente left join agentes t4 on t4.idagente = t1.idagente where t1.idusuario = '%d'",
            $sql = sprintf("SELECT idusuario,nombres , apellidos,concat(nombres, ' ', apellidos) As fullname, foto from usuarios where idusuario = '%d'", 
            mysql_real_escape_string($idusuario)
            );
            $result = mysql_query($sql);
            if(mysql_num_rows($result) > 0){
    ...
    public function usuario_autorizaciones($idusuario) {
        ...
        try {
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("SELECT t1.idusuario, t2.email, t1.autonomo, t1.idnivel1, t3.email as email1, t1.idnivel2, t4.email as email2, t1.idnivel3, t5.email as email3, t6.email as mailagente, t2.alternatemail, t2.asistenteemail, t2.copyemail FROM usuarios_autorizaciones t1 left join perfiles t2 on t2.idusuario = t1.idusuario left join perfiles t3 on t3.idusuario = t1.idnivel1 left join perfiles t4 on t4.idusuario = t1.idnivel2 left join perfiles t5 on t5.idusuario = t1.idnivel3 left join agentes t6 on t6.idagente = t2.idagente WHERE t1.idusuario = '%d'",
            mysql_real_escape_string($idusuario)
            );
            $result = mysql_query($sql);
            if(mysql_num_rows($result) > 0){
    ...
    public function carrier_buscar($idcarrier = null) {
        ...
        try {
            ...
            mysql_select_db($base, $conex);
            if ($idcarrier == null){
                $sql = sprintf("select idopercarrier, codigo, nombre, fechacreacion from oper_carrier order by nombre");
            } else { 
                $sql = sprintf("select idopercarrier, codigo, nombre, fechacreacion from oper_carrier where idopercarrier = '%d' order by nombre",
    ...
    public function perfil_nacionalidad_registrar( $idnacionalidad,$idperfil ,$pasaporte ,$tipo ,$fecha_emision , $fecha_expiracion, $secuenciapasaporte = null ) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("INSERT INTO perfil_nacionalidades
                            (idnacionalidad,
                            idperfil,
                            pasaporte,
                            tipo,
            ...
            mysql_real_escape_string($idperfil),
            mysql_real_escape_string($pasaporte),
            mysql_real_escape_string($tipo),
            mysql_real_escape_string($fecha_emision),
            mysql_real_escape_string($fecha_expiracion),
            mysql_real_escape_string($secuenciapasaporte)
            );
            mysql_query($sql);
            $result = mysql_insert_id();
            mysql_close();
            $this->registrar_evento('Registro de perfil_nacionalidad_registrar', $idusuario);
            //return null;
            return $result;
        }catch (Exception $ex){
            mysql_close();
    ...
    public function perfil_nacionalidad_modificar($id, $idnacionalidad ,$pasaporte ,$tipo ,$fecha_emision , $fecha_expiracion, $secuenciapasaporte = null ) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("UPDATE perfil_nacionalidades set 
                            idnacionalidad = '%s',
    ...
    public function perfil_visas_registrar($idperfil, $idnacionalidad, $idpaisvisa,$tipo ,$visa ,$fecha_emision , $fecha_expiracion,$comentario = null ) {
        include "config.php";
   if($server == '') {
        include "../config.php";
    }
    try {
            ...
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("INSERT INTO perfil_visas
                            (idperfil,
                            idnacionalidad,
                            idpaisvisa,
                            tipo,
            ...
            mysql_real_escape_string($idperfil), 
            mysql_real_escape_string($idnacionalidad),
            mysql_real_escape_string($idpaisvisa),
            mysql_real_escape_string($tipo),
            mysql_real_escape_string($visa),            
            mysql_real_escape_string($fecha_emision),
            mysql_real_escape_string($fecha_expiracion),
            ...
            $result = mysql_insert_id();
            mysql_close();
            $this->registrar_evento('Registro de perfil_visas_registrar', $idusuario);
            //return null;
            return $result;
        }catch (Exception $ex){
            mysql_close();
            $this->registrar_error('Error en registro de perfil_visas_registrar '.$idusuario, $ex, 'usuario_registrar', $idusuario);
            return null;
        }      
    }
    public function perfil_nacionalidad_inactivar($id){
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
    ...
    public function perfil_visa_inactivar($id){
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("UPDATE perfil_visas set 
                            activo = '0' 
                            where id = '%d'",            
            mysql_real_escape_string($id)
            );
    ...
    public function perfil_carrier_inactivar($id){
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("UPDATE perfil_viajero_frecuente set 
                            activo = '0' 
                            where id = '%d'",            
            mysql_real_escape_string($id)
            );
    ...
    public function perfil_visa_modificar($id, $idnacionalidad ,$idpaisvisa,$visa,$tipo ,$fecha_emision , $fecha_expiracion ,$comentario) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
    ...
    public function perfil_carrier_registrar($idperfil,$idcarrier , $numero, $fecha_emision,$fecha_expiracion , $millas, $comentario ) {
        include "config.php";
   if($server == '') {
        include "../config.php";
    }
    try {
            ...
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("INSERT INTO perfil_viajero_frecuente
                            (idperfil,
                            idcarrier,
                            numero,
                            fecha_emision,
            ...
            mysql_real_escape_string($fecha_emision),
            mysql_real_escape_string($fecha_expiracion),
            mysql_real_escape_string($millas),
            mysql_real_escape_string($comentario)                    
            );
            mysql_query($sql);
            $result = mysql_insert_id();
            mysql_close();
            $this->registrar_evento('Registro de perfil_carrier_registrar', $idusuario);
            //return null;
            return $result;
        }catch (Exception $ex){
            mysql_close();
            $this->registrar_error('Error en registro de perfil_carrier_registrar '.$idusuario, $ex, 'usuario_registrar', $idusuario);
            return null;
        }   
    }
    
    public function perfil_carrier_modificar($id, $id_carrier ,$numero,$fecha_emision,$fecha_expiracion ,$millas , $comentario) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            $sql = sprintf("UPDATE perfil_viajero_frecuente set 
                            ...
                            comentario = '%s'
                            where id = '%d'",
            mysql_real_escape_string($id_carrier), 
            //mysql_real_escape_string($idperfil),
            mysql_real_escape_string($numero),
            mysql_real_escape_string($fecha_emision),
            mysql_real_escape_string($fecha_expiracion),
    ...
    /* Informacion de sysusers */
    public function sysusers_buscar($idusuario,$idsysuser = null) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            mysql_select_db($base, $conex);
            if ($idsysuser == null){
                $sql = sprintf("SELECT idusuario, nombres, apellidos, concat(nombres, ' ', apellidos) As fullname,
                 usuario, password, email, cargo, fechaingreso, activo FROM usuarios where activo = '1'");
            } else { 
                $sql = sprintf("SELECT idusuario, nombres, apellidos, concat(nombres, ' ', apellidos) As fullname,
                 usuario, password, email, cargo, fechaingreso, activo FROM usuarios WHERE idusuario = '%d'",
                mysql_real_escape_string($idsysuser)
                );
            }
            ...
            if(mysql_num_rows($result) > 0){
                ...
                mysql_close();
                $this->registrar_evento('Carga de usuarios', $idusuario);
                return $arrayretorno;
            } else {
                $this->registrar_evento('Fallo de carga de usuarios, (user:'.$idusuario.')', $idusuario);
                mysql_close();
                return null;
            }
        }catch (Exception $ex){
            mysql_close();
            $this->registrar_error(': '.$idusuario, $ex, 'sysusers_buscar', $idusuario);
            return null;
        }
    }        
    
    public function sysusers_registrar($idusuario, $nombres, $apellidos, $usuario, $password,$email ,$cargo ,$foto, $activo ) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        ...
        try {
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("insert into usuarios (nombres, apellidos, usuario, password,email,cargo,foto, fechaingreso, activo) 
            values ('%s', '%s', '%s',md5('%s'),'%s','%s','%s',now(),'%d' )",
            mysql_real_escape_string($nombres),
            mysql_real_escape_string($apellidos),
            mysql_real_escape_string($usuario),
            mysql_real_escape_string($password),
            ...
            $result = mysql_insert_id();
            mysql_close();
            $this->registrar_evento('Registro de usuarios'.$result, $idusuario);
            return $result;
        }catch (Exception $ex){
            mysql_close();
            $this->registrar_error('Error en registro de usuarios: '.$idusuario, $ex, 'sysusers_registrar', $idusuario);
            return null;
        }        
    }
     
    public function sysusers_modificar($idusuario, $idsysuser , $nombres, $apellidos, $usuario, $password = null,$email ,$cargo ,  $activo ) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        ...
        try {                 
            ...
            mysql_select_db($base, $conex);
            if ($password == null) {
            $sql = sprintf("update usuarios set nombres = '%s', apellidos = '%s', usuario = '%s', email = '%s', cargo = '%s', activo = '%d' where idusuario = '%d'",
            mysql_real_escape_string($nombres),
            mysql_real_escape_string($apellidos),
            mysql_real_escape_string($usuario),
            mysql_real_escape_string($email),
            ...
            mysql_real_escape_string($idsysuser)
              ...
              $sql = sprintf("update usuarios set nombres = '%s', apellidos = '%s', usuario = '%s',password = md5('%s'), email = '%s', cargo = '%s', activo = '%d' where idusuario = '%d'",
            mysql_real_escape_string($nombres),
            mysql_real_escape_string($apellidos),
            mysql_real_escape_string($usuario),
            mysql_real_escape_string($password),
            ...
            $result = mysql_insert_id();
            mysql_close();
            $this->registrar_evento('modificacion de usuarios '.$result, $idusuario);
            return $result;
        }catch (Exception $ex){
            mysql_close();
            $this->registrar_error('Error en modificacion de usuarios: '.$idusuario, $ex, 'agentes_modificar', $idusuario);
            return "ERROR";
        }        
    }    
    
    public function sysusers_eliminar($idusuario, $idsysuser) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("delete from usuarios where idsysuser = '%d'",
            mysql_real_escape_string($idsysuser)
            );
            mysql_query($sql);
            $result = mysql_insert_id();
            mysql_close();
            $this->registrar_evento('eliminar usuarios '.$result, $idusuario);
            return $result;
        }catch (Exception $ex){
            mysql_close();
            $this->registrar_error('Error en eliminar usuarios: '.$idusuario, $ex, 'agentes_eliminar', $idusuario);
            return null;
        }        
    }        
    ...
    public function ciudades_buscar($idciudad = null, $idusuario = 0) {
        ...
        try {
            ...
            mysql_select_db($base, $conex);
            if ($idciudad == null){
                $sql = sprintf("select idciudad, concat(ciudad, ', ', codigo, ' (', iata, ')') As display, ciudad, iata, nombre, pais, codigo, area_code, region from oper_ciudades");
            } else { 
                $sql = sprintf("select idciudad, concat(ciudad, ', ', codigo, ' (', iata, ')') As display, ciudad, iata, nombre, pais, codigo, area_code, region from oper_ciudades where idcarrier = '%d'",
    ...
    /* Informacion de ciudades por pais */
    public function ciudades_pais_buscar($idnacionalidad = null, $idusuario = 0) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            if ($idnacionalidad == null){
                ...
                $sql = sprintf("SELECT t1.idciudad,
                    t1.ciudad,
                    t1.iata,
                    t1.nombre, 
                    t2.idnacionalidad,
                    ...
                    t1.region, 
                    t1.idagencia
                FROM oper_ciudades t1
                LEFT JOIN oper_nacionalidades t2 ON t2.pais = t1.pais 
                where t2.idnacionalidad = '%d'
    ...
    public function ciudades_buscar_display($idciudad = null, $idusuario = 0) {
        ...
        try {
            ...
            mysql_select_db($base, $conex);
            if ($idciudad == null){
                $sql = sprintf("select idciudad, concat(ciudad, ', ', codigo) As display from oper_ciudades order by idciudad asc,display");
            } else { 
                $sql = sprintf("select idciudad, concat(ciudad, ', ', codigo) As display from oper_ciudades where idcarrier = '%d' order by idciudad asc,display",
                mysql_real_escape_string($idciudad)
                );
            }
            $result = mysql_query($sql);
...
//            $result = mysql_insert_id();
    ...
    public function solicitud_flags_buscar($idoferta, $idusuario) {
        ...
        try {
            ...
            mysql_select_db($base, $conex);
            if ($idoferta == 0){
                $sql = sprintf("SELECT t1.idoferta, t1.idlinea, t1.idusuario, CONCAT(t2.nombres, ' ', t2.apellidos) AS nombre, t1.autorizado, t1.fecha_auth FROM reservas_oferta_flag t1 INNER join perfiles t2 ON t2.idusuario = t1.idusuario");
            } else { 
                $sql = sprintf("SELECT t1.idoferta, t1.idlinea, t1.idusuario, CONCAT(t2.nombres, ' ', t2.apellidos) AS nombre, t1.autorizado, t1.fecha_auth FROM reservas_oferta_flag t1 INNER join perfiles t2 ON t2.idusuario = t1.idusuario Where t1.idoferta = '%d'", 
                mysql_real_escape_string($idoferta)
                );
            }
            $result = mysql_query($sql);
...
//            $sql = sprintf("insert into reservas_oferta_det (idoferta, idsolicitud, idlinea, valor) values ('%d', '%d', '%d', '%s')",
    ...
    public function perfiles_buscar($idusuario) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            if ($idusuario == null) {
                /*
                $sql = sprintf("SELECT t1.idusuario, 
                    t1.nombres,                    
                    t1.apellidos,                    
                    t1.cargo,                    
                    t1.email,                    
                    t1.contactoemergencia,                    
                    t1.numeroemergencia,                    
                    t1.vacunafiebreamarilla,                    
                    t1.paisemision, 
                    t7.pais,  
                    t1.fechavencimiento,        
                    t1.intracompany,                    
                    ...
                    t3.nombre,                    
                    t1.idagente,                    
                    concat(t4.nombres,' ',t4.apellidos) as agente,                    
                    t1.idresidencia,                    
                    t5.pais,                    
                    t1.idciudad,                                       
                    t6.ciudad,                    
                    ...
                    t1.copyemail,  
                    t1.idasiento, 
                    t1.comentarioasiento, 
                    t1.fechanacimiento, 
                    t1.lugarnacimiento,
                    t1.fecharegistro
                FROM perfiles t1
                left join perfil_categoria t2 on t2.idcategoria = t1.idcategoria 
                left join clientes t3 on t3.idcliente = t1.idcliente
                left join agentes t4 on t4.idagente = t1.idagente
                left join oper_nacionalidades t5 on t5.idnacionalidad = t1.idresidencia
                left join oper_ciudades t6 on t6.idciudad = t1.idciudad
                left join oper_nacionalidades t7 on t7.idnacionalidad = t1.paisemision");  
                */
                $sql = sprintf("select * from todos_perfiles");
            } else {
                $sql = sprintf("SELECT 
                    t1.idusuario, 
                    t1.nombres,                    
    ...
    public function perfiles_buscar_side($idusuario) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            mysql_select_db($base, $conex);
            if ($idusuario == null) {
                $sql = sprintf("select * from todos_perfiles_side");
            } 
            $result = mysql_query($sql);
            //echo $sql;
            if(mysql_num_rows($result) > 0){
    ...
    public function perfiles_temp_buscar() { 
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
                $sql = sprintf("SELECT * FROM temp_perfiles");                
            ...
            if(mysql_num_rows($result) > 0){
                ...
                mysql_close();
                $this->registrar_evento('Carga de perfiles temporales', $idusuario);
                return $arrayretorno;
            } else {
                $this->registrar_evento('Fallo de carga de perfiles temporales, (user:'.$idusuario.')', $idusuario);
                mysql_close();
                return null;
            }
        }catch (Exception $ex){
    ...
    public function ciudades_temp_buscar($valorciudad, $valorpais) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
                $sql = "SELECT * FROM oper_ciudades where ciudad like '%".$valorciudad."%' and pais like '%".$valorpais."%' ";                
            ...
            if(mysql_num_rows($result) > 0){
                ...
                mysql_close();
                $this->registrar_evento('Carga de ciudades por coincidencia', $idusuario);
                return $arrayretorno;
            } else {
                $this->registrar_evento('Fallo de carga de ciudades por coincidencia, (user:'.$idusuario.')', $idusuario);
                mysql_close();
                return null;
            }
        }catch (Exception $ex){
    ...
    public function paises_temp_buscar($valor) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = "SELECT * FROM oper_nacionalidades where pais like '%" . $valor . "%'";
            $result = mysql_query($sql);
            if(mysql_num_rows($result) > 0){
                for($i = 0; $i < mysql_num_rows($result); $i++) {
                    $filas = mysql_fetch_array($result);
                ...
                mysql_close();
                $this->registrar_evento('Carga de coincidencia de paises', $idusuario);
                return $arrayretorno;
            } else {
                $this->registrar_evento('Fallo de carga de coincidencia de paises, (user:'.$idusuario.')', $idusuario);
                mysql_close();
                return null;
            }
        }catch (Exception $ex){
    ...
    /* BUSCAR EL PERFIL SEGUN LA EMPRESA */
    public function perfil_buscar_empresa($idempresa = null) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            mysql_select_db($base, $conex);
            if ($idempresa == null) {
              $sql = sprintf("SELECT t1.idusuario, t1.nombres, t1.apellidos , t1.cargo, t1.email, t1.idcliente, t2.nombre FROM perfiles t1 left join clientes t2 on t2.idcliente = t1.idcliente");
            } else { 
              $sql = sprintf("SELECT t1.idusuario, t1.nombres, t1.apellidos , t1.cargo, t1.email, t1.idcliente, t2.nombre FROM perfiles t1 left join clientes t2 on t2.idcliente = t1.idcliente where t1.idcliente = '%d'",
    ...
    public function perfil_buscar_nacionalidades($idperfil) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            mysql_select_db($base, $conex);
              ...
              $sql = sprintf("SELECT t1.id,
                        ...
                        t1.fecha_expiracion,
                        t1.idnacionalidad,
                        t1.secuencia_pasaporte
                        FROM perfil_nacionalidades t1
                        left join oper_nacionalidades t2 on t1.idnacionalidad = t2.idnacionalidad
                        WHERE t1.idperfil = '%d' and t1.activo = '1' 
                        order by t1.id desc",
               mysql_real_escape_string($idperfil)
               );  
    ...
    public function perfil_buscar_visas($idperfil) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            mysql_select_db($base, $conex);
              ...
              $sql = sprintf("SELECT t1.id,
                        ...
                        t1.idpaisvisa,
                        t1.comentario
                        FROM perfil_visas t1 
                        left join perfil_nacionalidades t2 on t1.idnacionalidad = t2.id
                        left join oper_nacionalidades t3 on t1.idpaisvisa = t3.idnacionalidad
                        left join oper_nacionalidades t4 on t2.idnacionalidad = t4.idnacionalidad
                        WHERE t1.idperfil = '%d' and t1.activo = '1'
                        order by t1.id desc",
               mysql_real_escape_string($idperfil)
               );  
    ...
    public function perfil_buscar_carriers($idperfil) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            mysql_select_db($base, $conex);
              ...
              $sql = sprintf("SELECT t1.id,
                        ...
                        t1.comentario,
                        t1.idcarrier 
                        FROM perfil_viajero_frecuente t1
                        left join oper_carrier t2 on t1.idcarrier = t2.idopercarrier
                        WHERE t1.idperfil = '%d' and t1.activo = '1'
                        order by t1.id desc",
               mysql_real_escape_string($idperfil)
               );  
    ...
    public function usuario_registrar($idusuario,
        ...
        try {
            ...
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("INSERT INTO perfiles
                            (nombres,apellidos,cargo,email,contactoemergencia,
                            numeroemergencia,vacunafiebreamarilla,paisemision,
                            fechavencimiento,intracompany,idcategoria,
                            idcliente,idagente,idresidencia,idciudad,alternatemail,
                            asistenteemail,copyemail,fecharegistro,idasiento,
                            comentarioasiento,fechanacimiento,lugarnacimiento,
                            contactoprechequeo,numeroprechequeo,codareanumpre,sexo,imagen)
                            VALUES
                            ('%s' ,'%s' ,'%s' ,'%s' ,'%s' ,
                             '%s' ,'%d' ,'%d' ,
                             '%s' ,'%d' ,'%d' ,
            ...
            mysql_real_escape_string($paisemision),
            mysql_real_escape_string($fechavencimiento), 
            mysql_real_escape_string($intracompany),
            mysql_real_escape_string($idcategoria),
            mysql_real_escape_string($idclientes),
            mysql_real_escape_string($idagente),
            mysql_real_escape_string($idresidencia),
            mysql_real_escape_string($idciudad),
            mysql_real_escape_string($alternatemail),
            mysql_real_escape_string($asistenteemail), 
            mysql_real_escape_string($copyemail),
            mysql_real_escape_string($idasiento),
            mysql_real_escape_string($comentarioasiento),
            mysql_real_escape_string($fechanacimiento),
            mysql_real_escape_string($lugarnacimiento),
            mysql_real_escape_string($contactoprechequeo),
            mysql_real_escape_string($numeroprechequeo),
            mysql_real_escape_string($codigoarea),
    ...
    public function usuario_modificar($idusuario,$id,
                                          $nombres ,$apellidos ,$cargo ,$email ,$contactoemergencia ,
                                          $numeroemergencia ,$vacunafiebreamarilla ,$paisemision ,
                                          $fechavencimiento ,$intracompany ,$idcategoria ,$idclientes ,
                                          $idagente ,$idresidencia ,$idciudad ,$alternatemail ,
                                          $asistenteemail ,$copyemail ,$idasiento,$comentarioasiento ,
                                          $fechanacimiento,$lugarnacimiento,$contactoprechequeo,$numeroprechequeo,$codigoarea,$sexo, $imagen = null) {
                                          //$fechanacimiento,$lugarnacimiento, $imagen = null) {
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            ...
            mysql_select_db($base, $conex);
            if ($imagen == null) {
                $sql = sprintf("UPDATE perfiles SET nombres = '%s', apellidos = '%s', cargo = '%s', email = '%s', contactoemergencia = '%s',
                                numeroemergencia = '%s',vacunafiebreamarilla = '%d',paisemision = '%d',fechavencimiento = '%s',
                                intracompany = '%d',idcategoria = '%d',idcliente = '%d',idagente = '%d',idresidencia = '%d',idciudad = '%d',
                                alternatemail ='%s',asistenteemail = '%s',copyemail = '%s',idasiento = '%s',
                                comentarioasiento = '%s',fechanacimiento = '%s',lugarnacimiento = '%s',
                                contactoprechequeo = '%s',numeroprechequeo = '%s',codareanumpre = '%s',sexo = '%s'   WHERE idusuario = '%d'",
                            mysql_real_escape_string($nombres),
                            mysql_real_escape_string($apellidos),
                            mysql_real_escape_string($cargo),
                            mysql_real_escape_string($email),
                ...
                $sql = sprintf("UPDATE perfiles SET nombres = '%s', apellidos = '%s', cargo = '%s', email = '%s', contactoemergencia = '%s',
                                ...
                                alternatemail ='%s',asistenteemail = '%s',copyemail = '%s',idasiento = '%s',
                                comentarioasiento = '%s',fechanacimiento = '%s',lugarnacimiento = '%s',
                                contactoprechequeo = '%s',numeroprechequeo = '%s',codareanumpre = '%s',imagen= '%s' WHERE idusuario = '%d'",
                            mysql_real_escape_string($nombres),
                            mysql_real_escape_string($apellidos),
                            mysql_real_escape_string($cargo),
                            mysql_real_escape_string($email),
    ...
    public function perfil_disable($idusuario, $id){
        include "config.php";
        if($server == '') {
            include "../config.php";
        }
        try {
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("UPDATE perfiles SET activo = '%d' WHERE idusuario = '%d'",                            
    ...
    public function usuario_autorizaciones_buscar($idusuario) {
        ...
        try {
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("SELECT t1.idusuario, t2.email, t1.autonomo, t1.idnivel1, t3.email as email1, t1.idnivel2, t4.email as email2, t1.idnivel3, t5.email as email3, t6.email as mailagente FROM usuarios_autorizaciones t1 left join perfiles t2 on t2.idusuario = t1.idusuario left join perfiles t3 on t3.idusuario = t1.idnivel1 left join perfiles t4 on t4.idusuario = t1.idnivel2 left join perfiles t5 on t5.idusuario = t1.idnivel3 left join agentes t6 on t6.idagente = t2.idagente WHERE t1.idusuario = '%d'",
            mysql_real_escape_string($idusuario)
            );
            $result = mysql_query($sql);
            if(mysql_num_rows($result) > 0){
    ...
    public function usuario_grupos_det_buscar($modobusqueda, $valor, $idusuario) {
        ...
        try {
            ...
            switch ($modobusqueda) { //0 = todos los usuarios, 1 = usuarios de grupo especifico, 2 = usuarios que no estan en ese grupo
                case 0:
                    $sql = sprintf("SELECT t1.idgrupo, t1.idusuario, concat(t2.nombres, ' ', t2.apellidos) as nombreusuario, t2.email, t1.fechaasignacion FROM usuarios_grupos_detalle t1 INNER join perfiles t2 on t2.idusuario = t1.idusuario");                                    
                    break;
                case 1:
                    $sql = sprintf("SELECT t1.idgrupo, t1.idusuario, concat(t2.nombres, ' ', t2.apellidos) as nombreusuario, t2.email, t1.fechaasignacion FROM usuarios_grupos_detalle t1 INNER join perfiles t2 on t2.idusuario = t1.idusuario WHERE t1.idgrupo = '%d'", 
                    mysql_real_escape_string($valor)
                    );                    
                    break;
                case 2:
                    $sql = sprintf("SELECT t2.idgrupo, t1.idusuario, CONCAT( t1.nombres,  ' ', t1.apellidos ) AS nombreusuario from perfiles t1 LEFT JOIN usuarios_grupos_detalle t2 ON t2.idusuario = t1.idusuario");             
                    break;
            }
    ...
    public function status_solicitudes($idusuario, $idpararegistro, $tipousuario, $idcliente = null) {
        ...
        try {
            ...
            switch($tipousuario){
                case 0:
                    $sql = sprintf("SELECT t1.idcliente, t1.idsolicitud, t1.idusuario, t1.fechacreacion, t1.anulada, t1.idcarrier, t2.nombre as carrier, t1.idproyecto, t3.nombre as proyecto, t8.nombre, concat(t9.nombres, ' ', t9.apellidos) as pasajero FROM solicitudes t1 LEFT JOIN oper_carrier t2 ON t2.codigo = t1.idcarrier LEFT JOIN clientes_proyectos t3 ON t3.idproyecto = t1.idproyecto LEFT JOIN clientes t8 ON t8.idcliente = t1.idcliente LEFT join perfiles t9 on t9.idusuario = t1.idusuario WHERE t1.anulada = 0 and NOT EXISTS (SELECT * FROM reservas_oferta t7 WHERE t7.idsolicitud = t1.idsolicitud)");
                    break;
                case 1:
                    $sql = sprintf("SELECT t1.idcliente, t1.idsolicitud, t1.idusuario, t1.fechacreacion, t1.anulada, t1.idcarrier, t2.nombre as carrier, t1.idproyecto, t3.nombre as proyecto, t8.nombre, concat(t9.nombres, ' ', t9.apellidos) as pasajero FROM solicitudes t1 LEFT JOIN oper_carrier t2 ON t2.codigo = t1.idcarrier LEFT JOIN clientes_proyectos t3 ON t3.idproyecto = t1.idproyecto LEFT JOIN clientes t8 ON t8.idcliente = t1.idcliente LEFT join perfiles t9 on t9.idusuario = t1.idusuario WHERE t1.anulada = 0 and t1.idcliente = '%d' AND NOT EXISTS (SELECT * FROM reservas_oferta t7 WHERE t7.idsolicitud = t1.idsolicitud)",
                        mysql_real_escape_string($idcliente)
                    );      
                    break;
                case 2:
                    ...
                    break;
                case 3:
                    $sql = sprintf("SELECT t1.idcliente, t1.idsolicitud, t1.idusuario, t1.fechacreacion, t1.anulada, t1.idcarrier, t2.nombre as carrier, t1.idproyecto, t3.nombre as proyecto, t8.nombre, concat(t9.nombres, ' ', t9.apellidos) as pasajero FROM solicitudes t1 LEFT JOIN oper_carrier t2 ON t2.codigo = t1.idcarrier LEFT JOIN clientes_proyectos t3 ON t3.idproyecto = t1.idproyecto LEFT JOIN clientes t8 ON t8.idcliente = t1.idcliente LEFT join perfiles t9 on t9.idusuario = t1.idusuario WHERE t1.anulada = 0 and t1.idusuario = '%d' AND NOT EXISTS (SELECT * FROM reservas_oferta t7 WHERE t7.idsolicitud = t1.idsolicitud)",
                        mysql_real_escape_string($idusuario)
                    );      
                    break;
                case 4:
                    $sql = sprintf("SELECT t1.idcliente, t1.idsolicitud, t1.idusuario, t1.fechacreacion, t1.anulada, t1.idcarrier, t2.nombre AS carrier, t1.idproyecto, t3.nombre AS proyecto, t4.idagente, t8.nombre, concat(t9.nombres, ' ', t9.apellidos) as pasajero FROM solicitudes t1 LEFT JOIN oper_carrier t2 ON t2.codigo = t1.idcarrier LEFT JOIN clientes_proyectos t3 ON t3.idproyecto = t1.idproyecto LEFT join perfiles t4 ON t4.idusuario = t1.idusuario LEFT JOIN clientes t8 ON t8.idcliente = t1.idcliente LEFT join perfiles t9 on t9.idusuario = t1.idusuario WHERE t1.anulada = 0 and t4.idagente = '%d' AND NOT EXISTS (SELECT * FROM reservas_oferta t7 WHERE t7.idsolicitud = t1.idsolicitud) LIMIT 0 , 30)",
                        mysql_real_escape_string($idusuario)
                    );      
                    break;
                case 5:
        ...
        try {
            ...
            switch($tipousuario){
                ...
                case 2:
                    ...
                    break;
                case 3:
                    $sql = sprintf("SELECT t1.idoferta, t1.idsolicitud, t1.idagente, t2.idusuario, t1.fechacreacion, t1.valoroferta, t1.titulooferta, t1.opciones, t1.detalle, t3.nombre, t1.idusuario, CONCAT( t4.nombres,  ' ', t4.apellidos ) AS usuario, t1.autorizada, t1.fecha_auth, t1.rechazada, t1.fecha_rech, t1.confirmada, t1.fechaconfirmacion, t1.finish, t1.fecha_finish, 0 as fullflags FROM reservas_oferta t1 INNER JOIN solicitudes t2 ON t2.idsolicitud = t1.idsolicitud INNER JOIN clientes t3 ON t3.idcliente = t2.idcliente INNER join perfiles t4 ON t1.idusuario = t4.idusuario Where t2.anulada = 0 and t2.idusuario = '%d'",
                        mysql_real_escape_string($idusuario)
                    );     
                    break;
                case 4:
                    $sql = sprintf("SELECT t1.idoferta, t1.idsolicitud, t1.idagente, t2.idusuario, t1.fechacreacion, t1.valoroferta, t1.titulooferta, t1.opciones, t1.detalle, t3.nombre, t1.idusuario, CONCAT( t4.nombres,  ' ', t4.apellidos ) AS usuario, t1.autorizada, t1.fecha_auth, t1.rechazada, t1.fecha_rech, t1.confirmada, t1.fechaconfirmacion, t1.finish, t1.fecha_finish, 0 as fullflags FROM reservas_oferta t1 INNER JOIN solicitudes t2 ON t2.idsolicitud = t1.idsolicitud INNER JOIN clientes t3 ON t3.idcliente = t2.idcliente INNER join perfiles t4 ON t1.idusuario = t4.idusuario Where t2.anulada = 0 and t1.idagente = '%d'",
                        mysql_real_escape_string($idusuario)
                    );      
                    break;
                case 5:
        ...
        try {
            ...
            switch($tipousuario){
                case 0:
                    ...
                    break;
                case 1:
                    $sql = sprintf("SELECT t1.idoferta, t1.idsolicitud, t1.idagente, t2.idusuario, t1.fechacreacion, t1.valoroferta, t1.titulooferta, t1.opciones, t1.detalle, t3.nombre, t1.idusuario, CONCAT( t4.nombres,  ' ', t4.apellidos ) AS usuario, t1.autorizada, t1.fecha_auth, t1.rechazada, t1.fecha_rech, t1.confirmada, t1.fechaconfirmacion, t1.finish, t1.fecha_finish FROM reservas_oferta_rechazadas t1 INNER JOIN solicitudes t2 ON t2.idsolicitud = t1.idsolicitud INNER JOIN clientes t3 ON t3.idcliente = t2.idcliente INNER join perfiles t4 ON t1.idusuario = t4.idusuario Where t2.idcliente = '%d'",
                        mysql_real_escape_string($idcliente)
                    ); 
                    break;
                case 2:
                    ...
                    break;
                case 3:
                    $sql = sprintf("SELECT t1.idoferta, t1.idsolicitud, t1.idagente, t2.idusuario, t1.fechacreacion, t1.valoroferta, t1.titulooferta, t1.opciones, t1.detalle, t3.nombre, t1.idusuario, CONCAT( t4.nombres,  ' ', t4.apellidos ) AS usuario, t1.autorizada, t1.fecha_auth, t1.rechazada, t1.fecha_rech, t1.confirmada, t1.fechaconfirmacion, t1.finish, t1.fecha_finish FROM reservas_oferta_rechazadas t1 INNER JOIN solicitudes t2 ON t2.idsolicitud = t1.idsolicitud INNER JOIN clientes t3 ON t3.idcliente = t2.idcliente INNER join perfiles t4 ON t1.idusuario = t4.idusuario Where t2.idusuario = '%d'",
                        mysql_real_escape_string($idusuario)
                    );     
                    break;
                case 4:
                    $sql = sprintf("SELECT t1.idoferta, t1.idsolicitud, t1.idagente, t2.idusuario, t1.fechacreacion, t1.valoroferta, t1.titulooferta, t1.opciones, t1.detalle, t3.nombre, t1.idusuario, CONCAT( t4.nombres,  ' ', t4.apellidos ) AS usuario, t1.autorizada, t1.fecha_auth, t1.rechazada, t1.fecha_rech, t1.confirmada, t1.fechaconfirmacion, t1.finish, t1.fecha_finish FROM reservas_oferta_rechazadas t1 INNER JOIN solicitudes t2 ON t2.idsolicitud = t1.idsolicitud INNER JOIN clientes t3 ON t3.idcliente = t2.idcliente INNER join perfiles t4 ON t1.idusuario = t4.idusuario Where t1.idagente = '%d'",
                        mysql_real_escape_string($idusuario)
                    );      
                    break;
                case 5:
        ...
        try {
            ...
            //$sql = sprintf("SELECT t1.idcliente, t1.idsolicitud, t1.idusuario, t1.fechacreacion, t1.anulada, t2.fecha_arri, t2.fecha_dep, t2.idcarrier, t3.nombre, t2.idcarrier, t4.ruta, t2.term_arri, CONCAT( t5.nombre,  ' (', t5.iata,  ')' ) AS nombre_term_arri, t2.term_dep, CONCAT( t6.nombre,  ' (', t6.iata,  ')' ) AS nombre_term_dep FROM solicitudes t1 LEFT JOIN solicitudes_detalle t2 ON t2.idsolicitud = t1.idsolicitud LEFT JOIN oper_carrier t3 ON t3.idcarrier = t2.idcarrier LEFT JOIN oper_rutas t4 ON t4.idruta = t2.idcarrier LEFT JOIN oper_ciudades t5 ON t5.idciudad = t2.term_arri LEFT JOIN oper_ciudades t6 ON t6.idciudad = t2.term_dep Where t1.idusuario = '%d' order by fechacreacion desc limit 0, 5",
                ...
                case 3:
                    ...
                    break;
                case 4:
                    $sql = sprintf("SELECT t1.idcliente, t1.idsolicitud, t1.idusuario, t1.fechacreacion, t1.anulada, t1.idcarrier, t2.nombre AS carrier, t1.idproyecto, t3.nombre AS proyecto, t4.idagente FROM solicitudes t1 LEFT JOIN oper_carrier t2 ON t2.codigo = t1.idcarrier LEFT JOIN clientes_proyectos t3 ON t3.idproyecto = t1.idproyecto LEFT join perfiles t4 ON t4.idcliente = t1.idcliente WHERE t1.anulada = 0 and t1.idagente = '%d' ORDER BY fechacreacion DESC LIMIT 0 , 5",
                        mysql_real_escape_string($idusuario)
                    );      
                    break;
                case 5:
    ...
    public function asignaciones_saldo($idusuario, $idasignacion, $valor){
        ...
        try {
            $conex = mysql_connect($server, $user, $pass);
            mysql_select_db($base, $conex);
            $sql = sprintf("update perfiles asignaciones set saldo = saldo - '%d' where idusuario = '%d' and idproyecto = '%d'",
                mysql_real_escape_string($valor),
                mysql_real_escape_string($idusuario),
